...
// utils file on ERP
/***************************************************************/
// this file contain token validation methods and sms methods
...
module.export = {
  ...
  validateAccessToken: async (req, res, next) => {
    let payload; 
    const autHeader = req.authorization || req.headers.Authorization;
  ...
  validateRefreshToien: async (req, res, next) => {
    let payload;
  ...
  validateSecretKey: (val) => {
    let id = CryptopJS.SHA256(process.env.Crypto_Val).toString(CryptoJS.enc.Hex)
  ...
  logout: async (refreshToken) => {
    ...
    if (!refreshTokenDoc) { return 'Not found' } else { await refreshTokenDoc.remove(); }
  },
  ...
  sms: async (num, message, token) => {
    ...
    const resMessage = await axios({
      method: 'post',
      url: 'https://smartmessagin.etisalat.ae:5676/campaigns/sumbissions/smsm/nb',
...
const verifyToken = async (token, type) => {
  ...
  if(!tokenDoc) { throw new Error('Token not found') } else { return tokenDoc }
}